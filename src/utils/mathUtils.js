...
import EquationInput from './EquationInput'; // Import your custom EquationInput component
...
function App() {
  ...
  const plotGraph = () => {
    try {
      const xValues = range(-10, 10, 0.5).toArray();  // Correct usage
      const yValues = xValues.map((x) => evaluate(expression, { x }));  // Correct usage
